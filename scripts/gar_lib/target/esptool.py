...
def validate_esp32_artifact(artifact_dir: Path) -> bool:
    if not artifact_dir.is_dir():
        print(f"gar target deploy: artifact dir not found: {artifact_dir}", file=sys.stderr)
        return False
    missing = [name for _, name in FLASH_LAYOUT if not (artifact_dir / name).is_file()]
    if missing:
        print(
            "gar target deploy: missing artifact file(s): " + ", ".join(missing),
            file=sys.stderr,
        )
        return False
    return True
...
def run_esp32_flash_command(
    *,
    ...
    if port_error:
        print(f"gar target deploy: {port_error}", file=sys.stderr)
        return 1
