...
Registry metadata is intentionally kept in ``registry/simulator/renode_mcu.py``.
"""
...
from scripts.gar_lib.environments.setup_option import DependencyStatus
...
def renode_dependency_status() -> list[DependencyStatus]:
...
def renode_install_hint(missing: list[str]) -> str:
    del missing
    return (
        "Renode が見つかりません。\n"
...
def install_renode_dependencies(missing: list[str]) -> int:
    if platform.system() != "Linux":
        print(renode_install_hint(missing))
        print(
...
def _select_portable_asset(assets: list[dict], arch: str) -> dict | None:
    ...
    if arch == "x86_64":
        generic = [a for a in candidates if not _name_has_token(a["name"], ("arm64", "aarch64"))]
        if generic:
...
def _is_linux_portable(name: str) -> bool:
    lower = name.lower()
    return "linux" in lower and "portable" in lower and (lower.endswith(".tar.gz") or lower.endswith(".tar.xz"))
...
def _install_portable(asset: dict, tag: str) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="gar-renode-") as tmp:
        ...
        except (tarfile.TarError, UnsafeArchiveError) as exc:
