...
- [x] **USB-C の auto-attach は on-demand attach で代替** — Windows タスクスケジューラで挿入瞬間に attach する常駐導線は不要と判断。`gar target app deploy` が必要時に `gar usb attach` 相当を自動実行するため、通常運用は gar 操作時の遅延 attach に集約 — 2026-06-06
- [x] **`gar target app deploy`（adb 経路）が実機未検出なら `gar usb attach` を自動先行実行する統合** — adb provider の deploy 前に `adb devices` を確認し、対象 device が見えない場合は `gar usb attach` 相当を自動実行してから再確認する形へ変更 — 2026-06-06
- [x] **旧 Windows PowerShell RasPi helper 相当（Codespace から成果物取得 → 実機 push）の `gar` 収容** — `gar target app fetch` で Codespace の artifact bundle を WSL hub へ取得し、現在は `gar target app deploy` が artifact 未取得時の fetch も担う — 2026-06-06 / 2026-07-03 更新
...
- [x] **panel 操作を `make panel-*` → `gar sim ...` へ移行**（現在の表記では `gar sim io press/set/clear`、`gar sim runtime status --json`。旧 `make panel-*` は削除し docs 主導線も更新。`make sim-test` は `gar sim` 直呼びに変更） — 2026-06-03
...
- [x] **`gar setup` に Renode(MCU/ベアメタル) simulation プロバイダを追加** — `scripts/gar_lib/environments/registry/simulator/renode_mcu.py`。Linux/WSL2 で最新 portable build を user-local 導入。プラグイン自動発見でコア無改修。ランタイム統合は未配線スタブ（`gar sim runtime` は当面 `ssh_remote` 利用） — 2026-06-10
