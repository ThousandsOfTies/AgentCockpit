...
- [x] **G1/G2** GPIO は `gpio-sim` + GPIO chardev v2 ルートに切替。`gar sim runtime start` が fake `/dev/gpiochip0` を用意し、`sensor_demo` は `/dev/gpiochip0` 固定のまま動く — 2026-06-03
...
- [x] **G2.5（実機検証）** EC2 で `gar sim runtime start` → `gar sim runtime diag --json` を流し、`/dev/gpiochip0` が gpio-sim 由来で本当に line を返すか確認 — 2026-06-03。`~/sensor_demo` 単体起動、Button17 到達、LED18 反映まで確認
- [x] **G4** GPIO の旧 shim を起動手順から削除 — 2026-06-03。gpio-sim + GPIO chardev v2 経路で `~/sensor_demo` 単体起動確認済み
- [x] **S4** `cuse_spi` 実装（MFRC-522 register sim を移植） — 2026-06-03。Codespace cross-build、EC2 deploy、`/dev/spidev0.0` 作成、`gar sim io set --device rfid --uid ...` で UID ログ、`gar sim io clear --device rfid` 復帰まで確認
- [x] **S5** SPI 旧 shim を削除、起動コマンドを `~/sensor_demo` のみに（S4 の実機確認が取れ次第） — 2026-06-03。追加環境変数なしの `~/sensor_demo` で GPIO/I2C/SPI 経路確認済み
- [x] **S6** docs の旧 shim 言及を「移行完了」に更新 — 2026-06-03。主要 docs は CUSE + gpio-sim 完了形へ更新
...
- [x] **旧 Windows PowerShell RasPi helper 相当（Codespace から成果物取得 → 実機 push）の `gar` 収容** — `gar target build`はbuild後にartifactを同期し、既存bundleの明示取得は`gar target fetch`、配置は`gar target deploy`が担当 — 2026-07-30更新
- [x] **GPIO ダミードライバの生成〜デプロイ手順を `gar` の操作単位に畳む** — `gar sim gpio plan/install/start/stop/status` を追加し、gpio-sim の生成計画、helper/service 配置、単体起動、状態確認を CLI 化。生の `modprobe` / configfs / bind mount 操作を避けられる導線へ整理 — 2026-06-05 / 2026-07-03 更新
- [x] **runtime socket 対応 artifact の再ビルド・EC2確認** — Codespace で `cuse_i2c` / `cuse_spi` を再生成し、`gar sim runtime deploy/start/diag --json` で `/run/gar/hw_sim.sock` 経路を確認。古い `/tmp/hw_sim.sock` を削除した状態で `~/sensor_demo`、Button17、RFID tap が通ることを確認 — 2026-06-05
- [x] **bridge / CUSE の runtime socket path 整理** — `bridge.py` / `cuse_i2c` / `cuse_spi` / 旧 `cuse_gpio` を `GAR_HW_SIM_SOCK` 優先、`GAR_RUNTIME_DIR/hw_sim.sock` 次点、`/tmp/hw_sim.sock` fallback に変更。`gar sim runtime start` の systemd unit から `GAR_HW_SIM_SOCK=/run/gar/hw_sim.sock` を渡す形へ更新 — 2026-06-05
- [x] **runtime 配置の本番寄せ** — `gar sim runtime deploy/start` を `/etc/gar/hardware/`、`/usr/local/sbin/`、`/usr/local/lib/gar/`、`/run/gar/` へ寄せ、systemd unit の `$HOME` 直参照を削除。旧 artifact manifest の `~/cuse_*` / `~/web-bridge` は deploy 側で互換マップ — 2026-06-05
- [x] **systemd 化 runtime の EC2 動作確認** — `gar sim runtime start` が `gar-sim.target` と `gar-gpio-sim.service` / `gar-bridge.service` / `gar-cuse-i2c@i2c-1.service` / `gar-cuse-spi@spidev0.0.service` を配置・起動する形へ移行。EC2 で4 service active、`gar sim runtime diag --json` `ok: true`、`~/sensor_demo` 起動、Button17 → `[btn] System ON`、RFID tap → UIDログ/OLED/state更新まで確認 — 2026-06-05
- [x] **hardware assignment CSV の PoC 導入** — `hardware/{components,gpio,i2c,spi,connections}.csv` を追加し、現在の GPIO17/18/24/27、I2C `/dev/i2c-1` (`0x3c` SSD1306 / `0x29` VL53L0X)、SPI `/dev/spidev0.0` (MFRC-522) を反映。`gar sim runtime start/diag` がCSVから GPIO line と I2C/SPI dev を読む形に変更 — 2026-06-05
- [x] **panel 操作を `make panel-*` → `gar sim ...` へ移行**（現在の表記では `gar sim io press/set/clear`、機械可読診断は`gar sim runtime diag --json`。旧 `make panel-*` は削除し docs 主導線も更新。`make sim-test` は `gar sim` 直呼びに変更） — 2026-07-30更新
- [x] `gar sim runtime diag --json` 実装（marker 区切り出力を `parse_sim_diag` で構造化、`ok`/exit code 判定、テスト追加） — 2026-06-03
- [x] `gar sim gpio check --json` 実装（modinfo/config.gz/configfs/kernel を構造化プローブ、Codex 移行分） — 2026-06-03
- [x] AGENT.md に「AI オペレーションの原則（契約）」追記（gar 経由・生コマンド連打回避・`--json`/exit code 確認・実機能確認まで done としない） — 2026-06-03
- [x] `info/02_DESIGN_PHILOSOPHY.md` に設計思想（なぜ仮想と実機を両方持つのか）を整理 — 2026-06-03
- [x] 旧 Windows PowerShell EC2 helper を `gar sim host start/stop/status` に移植（SSH config HostName 自動更新付き） — 2026-07-30表記更新
...
正本リポは `ThousandsOfTies/GaplessAgentRuntime`、ツールは `ThousandsOfTies/gar-tools`（`gar-build-env` / `gar-adhoc-app` も同 org）。
トップ 4 フォルダの物理リネームと連動するため、以下は **リネーム作業時に一括**で実施する。

- [x] **アプリフォルダの物理リネーム** — 新名称を `gar-adhoc-app` に統一し、GitHubリポジトリ、ローカルフォルダ、参照パスを更新 — 2026-07-31
- [x] **アプリ用product workspace作成** — `gar-build-env`の`GarAdhocApp` branchを`/home/user/Yurufuwa/GarAdhocApp`へcheckoutし、`sources/gar-adhoc-app`と`sources/gar-tools`をsubmodule化 — 2026-07-31
...
- [x] GPIO 解決方式の比較を `docs/06_SIMULATION.md` に整理
- [x] EC2 の `gpio-sim` 対応確認、`linux-modules-extra-$(uname -r)` 導入、`sensor_demo` GPIO v2 化、`gar sim runtime start` の fake `/dev/gpiochip0` setup 実装 — 2026-06-03
