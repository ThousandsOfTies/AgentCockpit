...
| **target** | 7 | 5 |  |  | 12 | 2 |  |  |  | 11 |  |  |
| **tests** | 15 | 6 | 7 | 39 | 74 | 18 | 15 |  | 39 | 13 |  | 1 |
| **vscode** |  |  |  |  | 3 |  |  |  |  |  |  |  |

file粒度 (116x116) の完全なmatrixは `GAR_LIB_DSM_file_level.csv` を参照 (Excel/sheetsで開くと見やすい)。
...
- 一意なfile依存edge数: 274
- 平均path長: 3.482
- import binding数による加重平均path長: 3.537
- p95 / 最大path長: 5 / 6

## 公開メンバの参照状況サマリ

- 対象module数: 116
- 公開メンバ(top-level関数/class/UPPER定数)総数: 429
- 他moduleから一度も参照されていないメンバ: 169  (詳細は `GAR_LIB_PUBLIC_API_USAGE.md` の「外部未参照」表)
